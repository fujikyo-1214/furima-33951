class PurchaseRecordAddress
  ...
  attr_accessor :prefecture_id, :post_number, :area, :area_number, :building, :number, :user_id, :item_id, :token

  with_options presence: true do
    validates :post_number, format: { with: /\A\d{3}[-]\d{4}\z/, message: "Input correctly" } 
    validates :area
    validates :area_number
    validates :number, length: { maximum: 11, message: "Input only number"}, format: { with: /\A[0-9]+\z/, message: "is not a number"}
    validates :token
    validates :user_id
    validates :item_id
  end
